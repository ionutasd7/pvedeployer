...
//Helper function to update dashboard statistics (added based on intention)
function updateDashboardStats() {
    fetch('/api/dashboard/stats') // Assumed API endpoint
        .then(response => response.json())
        .then(stats => {
            // Update UI elements with the received stats (example)
            document.getElementById('totalNodes').textContent = stats.nodes || 0; //Example: Update total nodes
            document.getElementById('totalDeployments').textContent = stats.deployments || 0; //Example: Update total deployments
            // Add more updates as needed based on your UI elements and API response
        })
    ...
    fetch('/api/nodes/status')
        ...
        .then(data => {
            ...
            if (nodeStatusList) {
                ...
                if (nodeCountElement) {
                    nodeCountElement.textContent = data.length;
                }
            }
